...
class ZoomService {
     ...
     */
    static registerProcessedUrl(url, filePath) {
        ...
        if (this.processedUrls.size >= this.MAX_CACHED_URLS) {
            ...
            if (oldestKey) {
                ...
                console.log(`🧹 Eliminando entrada más antigua del caché: ${oldRecord === null || oldRecord === void 0 ? void 0 : oldRecord.url}`);
     ...
     */
    static getProcessedUrlFile(url) {
        const urlHash = this.getUrlHash(url);
     ...
     */
    static async validateZoomUrl(zoomUrl) {
        let browser = null;
        try {
     ...
     */
    static async downloadZoomVideo(zoomUrl, downloadPath) {
     ...
     */
    static findExistingZoomVideos(directory) {
     ...
     */
    static async attemptVideoDownload(page) {
...
ZoomService.processedUrls = new Map();
// Máximo número de URLs a mantener en caché
ZoomService.MAX_CACHED_URLS = 5;
